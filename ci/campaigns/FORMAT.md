...
upstream-default three-library closure and 30-suite self-test run independently
from its symmetric-only configuration, 25-suite self-test, 140 generated suite
runners, 57 normal C sample/test programs, and ten standalone one-file fuzz
programs on the same two native Linux architectures. Curl's
