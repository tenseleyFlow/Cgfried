...
// ASM_CHECK-NOT(x86_64-linux-gnu): call{{[ \t]+}}__builtin_strdup
...
// ASM_CHECK-NOT(arm64-linux): bl{{[ \t]+}}__builtin_strdup
...
_Static_assert(__builtin_types_compatible_p(__typeof__(__builtin_strdup("abc")),
                                            char *),
               "strdup returns char pointer");
_Static_assert(__builtin_types_compatible_p(
...
int main(void)
{
    ...
    char *duplicate = __builtin_strdup("duplicate");
    char *joined =
    ...
    int copied = copy && copy[0] == 'a' && copy[1] == 'b' && copy[2] == 'c' &&
                 copy[3] == 0;
    int duplicated = duplicate && duplicate[0] == 'd' && duplicate[8] == 'e' &&
                     duplicate[9] == 0;
    ...
    printf("%d %d %d %d %d %d %d %d %d %d %d\n", found, padded_value, copied,
           duplicated, folded_case, joined_value, destination_calls == 1,
           source_calls == 1, count_calls == 1, end == padded + 2,
           joined == destination);
    __builtin_free(copy);
    __builtin_free(duplicate);
    return !found || !padded_value || !copied || !duplicated || !folded_case ||
