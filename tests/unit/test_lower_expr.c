...
void test_lower_builtin_strncmp_call_and_roundtrip(TestCtx *t)
{
    LowFix f;
    IrModule *round;
    ...
    T_ASSERT(t,
             run_lower(&f, "const char *left(void); const char *right(void); "
                           "__SIZE_TYPE__ count(void); "
                           "int use(void) { return "
                           "__builtin_strncmp(left(), right(), count()); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @left()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @right()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @count()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @strncmp(ptr"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "@__builtin_strncmp"), 0);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strncmp-external>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);

    T_ASSERT(t,
             run_lower(&f, "static int strncmp(const char *a, const char *b, "
                           "__SIZE_TYPE__ n) { "
                           "(void)a; (void)b; (void)n; return 7; } "
                           "int use(const char *a, const char *b) { "
                           "return __builtin_strncmp(a, b, 1); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @strncmp(ptr"), 1);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strncmp-defined>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
}
...
void test_lower_builtin_strspn_call_and_roundtrip(TestCtx *t)
{
    ...
    T_ASSERT(t, run_lower(&f, "const char *source(void); "
                              "const char *accept(void); "
                              "__SIZE_TYPE__ use(void) { return "
                              "__builtin_strspn(source(), accept()); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @source()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @accept()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @strspn(ptr"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "@__builtin_strspn"), 0);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strspn-external>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);

    T_ASSERT(t,
             run_lower(&f, "static __SIZE_TYPE__ strspn(const char *s, "
                           "const char *a) { (void)s; (void)a; return 7; } "
                           "__SIZE_TYPE__ use(const char *s, const char *a) { "
                           "return __builtin_strspn(s, a); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @strspn(ptr"), 1);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strspn-defined>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
}
...
void test_lower_builtin_strcpy_call_and_roundtrip(TestCtx *t)
{
    ...
    T_ASSERT(t,
             run_lower(&f, "char *destination(void); "
                           ...
                           "char *use(void) { return "
                           "__builtin_strcpy(destination(), source()); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @destination()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @source()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @strcpy(ptr"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "@__builtin_strcpy"), 0);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strcpy-external>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);

    T_ASSERT(t, run_lower(&f, "static char *strcpy(char *d, const char *s) { "
                              "(void)s; return d; } "
                              "char *use(char *d, const char *s) { "
    ...
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @strcpy(ptr"), 1);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strcpy-defined>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
}
...
void test_lower_builtin_bcopy_call_and_roundtrip(TestCtx *t)
{
    LowFix f;
    IrModule *round;

    T_ASSERT(t, run_lower(&f, "const void *source(void); "
                              "void *destination(void); "
                              "unsigned long count(void); "
                              "void use(void) { __builtin_bcopy("
    ...
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @destination()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @count()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @memmove(ptr"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "@bcopy"), 0);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<bcopy-external>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);

    T_ASSERT(t, run_lower(&f, "static void *memmove(void *d, "
                              "const void *s, unsigned long n) { "
                              "unsigned long i; for (i = 0; i < n; i++) "
                              "((char *)d)[i] = ((const char *)s)[i]; "
                              "return d; } "
                              "void use(const void *s, void *d) { "
                              "__builtin_bcopy(s, d, 3); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @memmove(ptr"), 1);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<bcopy-defined>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
}
...
void test_lower_builtin_strncpy_call_and_roundtrip(TestCtx *t)
{
    LowFix f;
    IrModule *round;

    T_ASSERT(t, run_lower(&f, "char *destination(void); "
                              "const char *source(void); int count(void); "
                              "char *use(void) { return __builtin_strncpy("
                              "destination(), source(), count()); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @destination()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @source()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @count()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "sext i32"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @strncpy(ptr"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "@__builtin_strncpy"), 0);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strncpy-external>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);

    T_ASSERT(t,
             run_lower(&f, "static char *strncpy(char *d, const char *s, "
                           "unsigned long n) { (void)s; (void)n; return d; } "
                           "char *use(char *d, const char *s) { return "
                           "__builtin_strncpy(d, s, 1); }\n"));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @strncpy(ptr"), 1);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strncpy-defined>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
    ...
    T_ASSERT(t, run_lower_opts(
                    ...
                    "char *use(char *d, const char *s) { return strncpy(d, s, "
                    "1); }\n",
                    STD_GNU17, false));
    T_ASSERT_EQ_INT(t, f.errors, 0);
    T_ASSERT(t, ir_verify(f.dc, f.m));
    ...
    T_ASSERT(t, f.m->funcs[0].always_inline);
    T_ASSERT(t, f.m->funcs[0].inline_only);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr external @strncpy(ptr"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @strncpy(ptr"), 1);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<strncpy-gnu-inline>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
}
...
void test_lower_builtin_popcount_family(TestCtx *t)
{
    static const char calls[] =
        "unsigned source_i(void); unsigned long source_l(void); "
        "unsigned long long source_ll(void); int use(void) { return "
        "__builtin_popcount(source_i()) + __builtin_popcountl(source_l()) + "
        "__builtin_popcountll(source_ll()); }\n";
    LowFix f;
    IrModule *round;
    ...
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @source_l()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @source_ll()"), 1);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @popcount("), 0);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @popcountl("), 0);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @popcountll("), 0);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "lshr i32"), 5);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "lshr i64"), 12);
    T_ASSERT_EQ_INT(t, count_of(txt(&f), "trunc i64"), 2);
    round = ir_parse_module(&f.arena, f.dc, txt(&f), "<popcount-family>");
    T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
    low_free(&f);
...
void test_lower_builtin_checked_overflow_predicate_family(TestCtx *t)
{
    static const char source[] =
        "unsigned add_left(void); long long add_right(void); "
        "signed char add_selector(void); "
        "long sub_left(void); unsigned long sub_right(void); "
        "unsigned short sub_selector(void); "
        "unsigned short mul_left(void); long mul_right(void); "
        "struct bits { signed int value : 5; }; "
        "struct bits *mul_selector(void); "
        "_Bool add(void) { return __builtin_add_overflow_p(add_left(), "
        "add_right(), add_selector()); } "
        "_Bool sub(void) { return __builtin_sub_overflow_p(sub_left(), "
        "sub_right(), sub_selector()); } "
        "_Bool mul(void) { return __builtin_mul_overflow_p(mul_left(), "
        "mul_right(), mul_selector()->value); }\n";
    static const TargetKind targets[] = {CGF_TARGET_X86_64_LINUX_GNU,
                                         CGF_TARGET_ARM64_LINUX};
    LowFix f;
    size_t i;
    ...
    for (i = 0; i < CGF_ARRAY_LEN(targets); i++) {
        ...
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i32 @add_left()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @add_right()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i8 @add_selector()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @sub_left()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @sub_right()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i16 @sub_selector()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i16 @mul_left()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call i64 @mul_right()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "call ptr @mul_selector()"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "store "), 0);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "udiv i64"), 1);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), " nsw"), 0);
        T_ASSERT_EQ_INT(t, count_of(txt(&f), "@__builtin_"), 0);
        round = ir_parse_module(&f.arena, f.dc, txt(&f), "<overflow-p-family>");
        T_ASSERT(t, round != NULL && ir_module_struct_eq(f.m, round));
        low_free(&f);
    }
}
