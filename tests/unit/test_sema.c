...
void test_sema_builtin_strcspn_contract(TestCtx *t)
{
    SemaFix f;

    run_sema(&f,
             "_Static_assert(_Generic(__builtin_strcspn(\"abc\", \"c\"), "
             "__SIZE_TYPE__: 1, default: 0), \"strcspn result is size_t\"); "
             "__SIZE_TYPE__ f(char *s, const char *reject, void *p) { "
    ...
    run_sema(&f, "__SIZE_TYPE__ f(char *s) { return __builtin_strcspn(s); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(
        &f, "__SIZE_TYPE__ f(char *s) { return __builtin_strcspn(s, s, s); }\n",
        STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);
...
void test_sema_builtin_stpcpy_contract(TestCtx *t)
{
    SemaFix f;

    run_sema(&f,
             "_Static_assert(_Generic(__builtin_stpcpy((char *)0, \"x\"), "
             "char *: 1, default: 0), \"stpcpy result is char pointer\"); "
             "char *f(char *d, const char *s, void *p) { "
             "char *a = __builtin_stpcpy(d, s); "
             "char *b = __builtin_stpcpy(p, s); return a ? a : b; }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 0);
    sfix_free(&f);

    run_sema(&f, "char *f(char *p) { return __builtin_stpcpy(p); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(&f, "char *f(char *p) { return __builtin_stpcpy(p, p, p); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(&f,
             "struct S { int x; }; char *f(struct S s, const char *p) { "
             "return __builtin_stpcpy(s, p); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);
...
void test_sema_builtin_strchr_contract(TestCtx *t)
{
    ...
    run_sema(&f,
             "_Static_assert(_Generic(__builtin_strchr(\"abc\", 'b'), "
             "char *: 1, default: 0), \"strchr result is char pointer\"); "
             "char *f(const char *s, void *p, long c) { "
             "char *a = __builtin_strchr(s, c); "
             "char *b = __builtin_strchr(p, 0); return a ? a : b; }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 0);
    sfix_free(&f);

    run_sema(&f, "char *f(const char *s) { return __builtin_strchr(s); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(&f,
             "char *f(const char *s) { return __builtin_strchr(s, 0, 1); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);
    ...
    run_sema(&f,
             "struct S { int x; }; char *f(const char *s, struct S v) { "
             "return __builtin_strchr(s, v); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);
}

void test_sema_builtin_strncpy_contract(TestCtx *t)
{
    SemaFix f;

    run_sema(&f,
             "_Static_assert(_Generic(__builtin_strncpy((char *)0, \"x\", "
             "1), char *: 1, default: 0), \"strncpy result is char pointer\"); "
             "char *f(char *d, const char *s, void *p, int n) { "
             "char *a = __builtin_strncpy(d, s, n); "
             "char *b = __builtin_strncpy(p, s, 1); return a ? a : b; }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 0);
    sfix_free(&f);
    ...
    run_sema(&f,
             "char *f(char *d, const char *s) { "
             "return __builtin_strncpy(d, s, 1, 2); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(&f,
             "struct S { int x; }; char *f(struct S v, const char *s) { "
             "return __builtin_strncpy(v, s, 1); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(&f,
             "struct S { int x; }; char *f(char *d, struct S v) { "
             "return __builtin_strncpy(d, v, 1); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);

    run_sema(&f,
             "struct S { int x; }; char *f(char *d, const char *s, "
             "struct S v) { return __builtin_strncpy(d, s, v); }\n",
             STD_GNU17);
    T_ASSERT_EQ_INT(t, f.errors, 1);
    sfix_free(&f);
}
...
void test_sema_bitfield_integer_promotions(TestCtx *t)
{
    ...
    run_sema(
        ...
        "unsigned long long ull35 : 35; unsigned long long ull40 : 40; "
        "signed long long s40 : 40; };\n"
        "#define SAME(E, T) __builtin_types_compatible_p(__typeof__(E), T)\n"
        "#define COMPAT(E, F) __builtin_types_compatible_p(__typeof__(E), "
        "__typeof__(F))\n"
        ...
        "\"ull35 is extended\");\n"
        "_Static_assert(COMPAT(((struct B *)0)->ull35 + 1, "
        "+((struct B *)0)->ull35), \"extended plus int\");\n"
        "_Static_assert(COMPAT(((struct B *)0)->ull35 + "
        "((struct B *)0)->ull40, +((struct B *)0)->ull40), "
        "\"wider extended wins\");\n"
        "_Static_assert(SAME(((struct B *)0)->ull40 + 1L, long), "
        "\"standard long wins\");\n"
        "_Static_assert(COMPAT(((struct B *)0)->s40 + 1U, "
        "+((struct B *)0)->s40), \"signed extended holds uint\");\n"
        "_Static_assert(COMPAT(((struct B *)0)->s40 + "
        "((struct B *)0)->ull40, +((struct B *)0)->ull40), "
        "\"unsigned wins rank tie\");\n"
        "_Static_assert(!SAME((0, ((struct B *)0)->ull40), "
        "unsigned long long), \"comma preserves field precision\");\n"
        "_Static_assert(SAME((unsigned long long)((struct B *)0)->ull40, "
        "unsigned long long), \"explicit cast ends precision\");\n"
