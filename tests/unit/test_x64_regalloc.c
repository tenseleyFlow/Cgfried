...
static u64 sim_sext(u64 v, u8 bytes)
{
    if (bytes == 1)
        return (u64)(i64)(i8)v;
    if (bytes == 2)
...
static u64 sim_mask(u64 v, u8 w)
{
    return w == 8 ? v : v & ((1ull << (w * 8)) - 1);
}

static u64 sim_addr(TestCtx *t, const Sim *s, const X64Mem *m)
{
    u64 a = 0;
...
static u64 sim_rdmem(TestCtx *t, const Sim *s, u64 addr, u8 w)
{
    u64 v = 0;
    u8 i;
...
static void sim_wrmem(TestCtx *t, Sim *s, u64 addr, u64 v, u8 w)
{
    u8 i;
...
static u64 sim_src(TestCtx *t, const Sim *s, const X64Operand *o, u8 w)
{
    switch (o->kind) {
    case X64O_VREG:
        return s->val[o->r.v];
    case X64O_IMM:
        return (u64)o->imm;
    case X64O_MEM:
        return sim_rdmem(t, s, sim_addr(t, s, &o->mem), w);
    default:
        T_ASSERT(t, false);
        return 0;
    }
...
/* Executes block 0 until RET. Returns false on an op it cannot model. */
static bool sim_run(TestCtx *t, const X64Func *f, Sim *s)
{
    const X64Block *b = &f->blocks[0];
    u32 i;
    ...
    for (i = 0; i < b->n; i++) {
        ...
        case X64_OP_MOVABS:
            ...
            break;
        case X64_OP_MOVZX:
            s->val[in->def.v] = sim_mask(s->val[in->a.r.v], in->src_width);
            break;
        case X64_OP_MOVSX:
            s->val[in->def.v] =
                sim_mask(sim_sext(s->val[in->a.r.v], in->src_width), w);
            break;
        case X64_OP_LEA:
    ...
    return false; /* fell off without ret */
}
...
void test_x64_regalloc_interp_differential(TestCtx *t)
{
    ...
    for (iter = 0; iter < 200; iter++) {
        ...
        s1.mem = mem1;
        s2.mem = mem2;
        memset(mem1, 0, SIM_MEM);
        memset(mem2, 0, SIM_MEM);
        s2.val[X64_RSP + 1] = SIM_MEM - 256; /* stack top */
        ok1 = sim_run(t, f1, &s1);
        ok2 = sim_run(t, f2, &s2);
        T_ASSERT(t, ok1);
        T_ASSERT(t, ok2);
