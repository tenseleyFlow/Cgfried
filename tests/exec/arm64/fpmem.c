...
static long roundtrip_ref(long n)
{
    return (long)((double)n * 2.5);
}
static float narrow_ref(double x)
{
    return (float)x * 3.0f;
}
static long globals_ref(long v)
{
    return v + v * 3;
}
...
int main(void)
{
    ...
    for (n = -50; n < 50; n++)
        chk("globals", globals(n), globals_ref(n));
