...
int main(void)
{
    ...
    for (i = -3; i < 40; i++) chk("sumto", sumto(i), sumto_ref(i));
    for (i = -5; i < 10; i++) chk("classify", classify((int)i), classify_ref((int)i));
    for (i = -4; i < 5; i++)
        for (j = -4; j < 5; j++) chk("nested", nested(i, j), nested_ref(i, j));
