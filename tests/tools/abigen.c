 ...
 *     A <type>     one per argument, in order
 * Types:
...
static void print_type(FILE *f, const Type *t)
{
    ...
    switch (t->kind) {
    case K_CHAR: fputc('c', f); return;
    case K_SHORT: fputc('h', f); return;
    case K_INT: fputc('i', f); return;
    case K_LONG: fputc('l', f); return;
    case K_FLOAT: fputc('f', f); return;
    case K_DOUBLE: fputc('d', f); return;
    case K_VOID: fputc('v', f); return;
    case K_ARRAY:
...
static Type *parse_type(void)
{
    ...
    while (*pp == ' ')
        pp++;
    switch (*pp) {
    case 'c': pp++; return tnew(K_CHAR);
    case 'h': pp++; return tnew(K_SHORT);
    case 'i': pp++; return tnew(K_INT);
    case 'l': pp++; return tnew(K_LONG);
    case 'f': pp++; return tnew(K_FLOAT);
    case 'd': pp++; return tnew(K_DOUBLE);
    case 'v': pp++; return tnew(K_VOID);
    case 'a':
        pp += 2; /* "a(" */
        t = tnew(K_ARRAY);
...
{
    switch (k) {
    case K_CHAR: return 0x3Full;
    case K_SHORT: return 0x3FFFull;
    case K_INT: return 0x3FFFFFFFull;
    case K_FLOAT: return 0xFFFFFFull;             /* 24-bit mantissa */
    case K_DOUBLE: return 0x1FFFFFFFFFFFFFull;    /* 53-bit mantissa */
    default: return 0x3FFFFFFFFFFFFFFFull;
    }
}
...
static void emit_typedefs(FILE *f, Type *t, int *id)
{
    ...
    for (i = 0; i < t->nmem; i++) {
        ...
        if (m->kind == K_ARRAY) {
            fprintf(f, " ");
            switch (m->mem[0]->kind) {
            case K_CHAR: fprintf(f, "char"); break;
            case K_SHORT: fprintf(f, "short"); break;
            case K_INT: fprintf(f, "int"); break;
            case K_LONG: fprintf(f, "long"); break;
            case K_FLOAT: fprintf(f, "float"); break;
            default: fprintf(f, "double"); break;
            }
...
int main(int argc, char **argv)
{
    ...
    if (simplify >= 0) {
        ...
        load_desc(emit_file, &ret, args, &na);
