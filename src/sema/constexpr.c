...
static ConstValue eval(Sema *s, AstNode *e, CeMode m)
{
    ...
    case AST_EXPR_CALL: {
        ...
        if ((e->op == SEMA_BUILTIN_CLRSB || e->op == SEMA_BUILTIN_CLRSBL ||
            ...
            /* Sema has converted the operand to the builtin's exact signed
             * prototype type. Count consecutive bits below its sign bit that
             * equal that sign; zero and -1 therefore both return width - 1. */
            if (a.kind != CV_INT || width == 0)
                return cv_error();
            sign = ((a.i >> (width - 1)) & 1) != 0;
            for (bit = width - 1; bit > 0; bit--) {
                bool next = ((a.i >> (bit - 1)) & 1) != 0;
        ...
        if ((e->op == SEMA_BUILTIN_POPCOUNT ||
             ...
             e->op == SEMA_BUILTIN_POPCOUNTLL || e->op == SEMA_BUILTIN_PARITY ||
            ...
            u32 count = 0;
            u32 bit;
            bool parity = e->op == SEMA_BUILTIN_PARITY ||
                          e->op == SEMA_BUILTIN_PARITYL ||
                          e->op == SEMA_BUILTIN_PARITYLL;

            /* Sema has converted the operand to the builtin's exact unsigned
             * prototype type. Count that target-width bit image and return
             * either the count or its low bit as int; unlike clz/ctz, zero is
             * defined and naturally produces zero. */
            if (a.kind != CV_INT || width == 0)
                return cv_error();
            for (bit = 0; bit < width; bit++)
                if ((a.i & (1ull << bit)) != 0)
