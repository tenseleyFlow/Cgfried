 ...
 * the zero-scan, population-count, and parity families use its unsigned
 * counterpart. Keep that mapping beside call typing so adding one spelling
 * cannot accidentally inherit the default argument promotions. */
...
/* These binary floating builtins have ordinary fixed prototypes, not
 * type-generic operands: each argument converts to the declared format. */
static Type *builtin_float_binary_param_type(u16 builtin)
{
    switch (builtin) {
    case SEMA_BUILTIN_COPYSIGN:
    case SEMA_BUILTIN_POW:
        return type_basic(TY_DOUBLE);
...
static u32 builtin_formatted_output_fixed_args(u16 builtin)
{
    switch (builtin) {
...
static const char *builtin_formatted_output_name(u16 builtin)
{
    switch (builtin) {
    case SEMA_BUILTIN_PRINTF:
        return "printf";
        ...
        if (b) {
            ...
            if (formatted_fixed && e->nargs < formatted_fixed) {
                err(s, e->span, "'%s' takes at least %u argument%s",
                    ...
                    formatted_fixed == 1 ? "" : "s");
                return poison(s, e);
            } else if (b == SEMA_BUILTIN_VA_START &&
                       (e->nargs < 1 || e->nargs > 2)) {
            ...
            if (formatted_fixed) {
                ...
                if (b == SEMA_BUILTIN_SPRINTF) {
                    params[0] = charp;
                    params[1] = const_charp;
                } else if (b == SEMA_BUILTIN_SNPRINTF) {
                    params[0] = charp;
                ...
                for (; i < e->nargs; i++) {
                    ...
                    if (arg->sem_type && arg->sem_type->kind == TY_VOID) {
                        err(s, arg->span, "invalid use of void expression");
                        e->args[i] = poison(s, arg);
                        valid = false;
                        continue;
                    }
                    e->args[i] = conv_promote(s, arg);
                    if (e->args[i]->sem_type->kind == TY_FLOAT)
                        e->args[i] =
                            conv_cast(s, e->args[i], type_basic(TY_DOUBLE));
                }
                if (!valid)
                    return poison(s, e);
            }
             ...
             * these to real libc calls — inlining is Phase 7/11). */
            {
                ...
                if (b == SEMA_BUILTIN_FREE || b == SEMA_BUILTIN_STACK_RESTORE ||
                ...
                if (b == SEMA_BUILTIN_STRCMP || b == SEMA_BUILTIN_STRSPN ||
                    b == SEMA_BUILTIN_STRCSPN || b == SEMA_BUILTIN_STRSTR) {
                ...
                if (b == SEMA_BUILTIN_STRCPY || b == SEMA_BUILTIN_STPCPY ||
                    ...
                    Type *params[] = {charp, type_ptr(s->arena, const_char),
                                      type_basic(TY_ULONG)};
                    u32 nparams = b == SEMA_BUILTIN_STPCPY_CHK ||
                 ...
                 * the way. Promoting it instead would swap the wrong bytes. */
                {
                    ...
                    Type *float_binary_type =
                        builtin_float_binary_param_type(b);
                    ...
                    } else if (float_binary_type) {
                        for (i = 0; i < e->nargs; i++) {
                            bctx.arg_index = i + 1;
                            conv_assignable(s, float_binary_type, &e->args[i],
                                            bctx);
                        }
