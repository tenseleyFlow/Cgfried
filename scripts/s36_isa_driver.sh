...
if [ "$corpus_count" -ne 82 ]; then
    echo "s36_isa_driver: expected 82 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 492 ]; then
    echo "s36_isa_driver: expected exactly 492 object checks, ran $checks" >&2
    exit 1
fi
