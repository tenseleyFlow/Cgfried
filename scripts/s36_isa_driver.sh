...
if [ "$corpus_count" -ne 74 ]; then
    echo "s36_isa_driver: expected 74 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 444 ]; then
    echo "s36_isa_driver: expected exactly 444 object checks, ran $checks" >&2
    exit 1
fi
