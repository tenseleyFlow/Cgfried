...
if [ "$corpus_count" -ne 78 ]; then
    echo "s36_isa_driver: expected 78 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 468 ]; then
    echo "s36_isa_driver: expected exactly 468 object checks, ran $checks" >&2
    exit 1
fi
