...
if [ "$corpus_count" -ne 71 ]; then
    echo "s36_isa_driver: expected 71 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 426 ]; then
    echo "s36_isa_driver: expected exactly 426 object checks, ran $checks" >&2
    exit 1
fi
