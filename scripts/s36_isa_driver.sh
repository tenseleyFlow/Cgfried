...
if [ "$corpus_count" -ne 77 ]; then
    echo "s36_isa_driver: expected 77 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 462 ]; then
    echo "s36_isa_driver: expected exactly 462 object checks, ran $checks" >&2
    exit 1
fi
