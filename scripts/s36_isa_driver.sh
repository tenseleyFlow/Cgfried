...
if [ "$corpus_count" -ne 79 ]; then
    echo "s36_isa_driver: expected 79 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 474 ]; then
    echo "s36_isa_driver: expected exactly 474 object checks, ran $checks" >&2
    exit 1
fi
