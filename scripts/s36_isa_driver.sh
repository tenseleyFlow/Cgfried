...
if [ "$corpus_count" -ne 84 ]; then
    echo "s36_isa_driver: expected 84 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 504 ]; then
    echo "s36_isa_driver: expected exactly 504 object checks, ran $checks" >&2
    exit 1
fi
