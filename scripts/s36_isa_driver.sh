...
if [ "$corpus_count" -ne 98 ]; then
    echo "s36_isa_driver: expected 98 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 588 ]; then
    echo "s36_isa_driver: expected exactly 588 object checks, ran $checks" >&2
    exit 1
fi
