...
if [ "$corpus_count" -ne 83 ]; then
    echo "s36_isa_driver: expected 83 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 498 ]; then
    echo "s36_isa_driver: expected exactly 498 object checks, ran $checks" >&2
    exit 1
fi
