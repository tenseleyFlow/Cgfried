...
if [ "$corpus_count" -ne 87 ]; then
    echo "s36_isa_driver: expected 87 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 522 ]; then
    echo "s36_isa_driver: expected exactly 522 object checks, ran $checks" >&2
    exit 1
fi
