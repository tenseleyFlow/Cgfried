...
if [ "$corpus_count" -ne 68 ]; then
    echo "s36_isa_driver: expected 68 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 408 ]; then
    echo "s36_isa_driver: expected exactly 408 object checks, ran $checks" >&2
    exit 1
fi
