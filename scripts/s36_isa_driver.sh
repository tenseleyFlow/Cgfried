...
if [ "$corpus_count" -ne 85 ]; then
    echo "s36_isa_driver: expected 85 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 510 ]; then
    echo "s36_isa_driver: expected exactly 510 object checks, ran $checks" >&2
    exit 1
fi
