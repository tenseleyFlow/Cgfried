...
if [ "$corpus_count" -ne 100 ]; then
    echo "s36_isa_driver: expected 100 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 600 ]; then
    echo "s36_isa_driver: expected exactly 600 object checks, ran $checks" >&2
    exit 1
fi
