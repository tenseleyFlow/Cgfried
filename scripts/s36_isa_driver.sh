...
if [ "$corpus_count" -ne 90 ]; then
    echo "s36_isa_driver: expected 90 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 540 ]; then
    echo "s36_isa_driver: expected exactly 540 object checks, ran $checks" >&2
    exit 1
fi
