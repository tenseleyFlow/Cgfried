...
if [ "$corpus_count" -ne 94 ]; then
    echo "s36_isa_driver: expected 94 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 564 ]; then
    echo "s36_isa_driver: expected exactly 564 object checks, ran $checks" >&2
    exit 1
fi
