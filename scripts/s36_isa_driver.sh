...
if [ "$corpus_count" -ne 73 ]; then
    echo "s36_isa_driver: expected 73 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 438 ]; then
    echo "s36_isa_driver: expected exactly 438 object checks, ran $checks" >&2
    exit 1
fi
