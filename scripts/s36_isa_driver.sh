...
if [ "$corpus_count" -ne 70 ]; then
    echo "s36_isa_driver: expected 70 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 420 ]; then
    echo "s36_isa_driver: expected exactly 420 object checks, ran $checks" >&2
    exit 1
fi
