...
if [ "$corpus_count" -ne 80 ]; then
    echo "s36_isa_driver: expected 80 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 480 ]; then
    echo "s36_isa_driver: expected exactly 480 object checks, ran $checks" >&2
    exit 1
fi
