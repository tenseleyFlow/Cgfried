...
if [ "$corpus_count" -ne 92 ]; then
    echo "s36_isa_driver: expected 92 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 552 ]; then
    echo "s36_isa_driver: expected exactly 552 object checks, ran $checks" >&2
    exit 1
fi
