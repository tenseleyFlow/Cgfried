...
if [ "$corpus_count" -ne 69 ]; then
    echo "s36_isa_driver: expected 69 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 414 ]; then
    echo "s36_isa_driver: expected exactly 414 object checks, ran $checks" >&2
    exit 1
fi
