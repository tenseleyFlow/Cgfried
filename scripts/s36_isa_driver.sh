...
if [ "$corpus_count" -ne 86 ]; then
    echo "s36_isa_driver: expected 86 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 516 ]; then
    echo "s36_isa_driver: expected exactly 516 object checks, ran $checks" >&2
    exit 1
fi
