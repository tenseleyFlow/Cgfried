...
if [ "$corpus_count" -ne 95 ]; then
    echo "s36_isa_driver: expected 95 corpus C files, found $corpus_count" >&2
    exit 1
fi
...
if [ "$checks" -ne 570 ]; then
    echo "s36_isa_driver: expected exactly 570 object checks, ran $checks" >&2
    exit 1
fi
