...
# The compiler spawns `as` and `ld`. Under qemu those exec calls reach the
# HOST, so they must be routed to the cross tools by absolute path -- left
# alone they would silently produce x86 objects.
