...
control_script=${CGF_FLEET_BENCH_CONTROL:-$(CDPATH='' cd "$(dirname "$0")" && pwd -P)/bench-control.sh}
...
[ -x "$control_script" ] || { echo "fleet-bench: control helper is not executable: $control_script" >&2; exit 3; }
...
result_host=$(sed -n 's/^host=//p' "$result")
[ "$result_host" = "$host" ] &&
    [ "$(printf '%s\n' "$result_host" | wc -l)" -eq 1 ] || {
    echo 'fleet-bench: result has no unique matching host provenance' >&2
    exit 3
}
...
control_status=0
control_state=$("$control_script" classify --require-v2 "$result") ||
    control_status=$?
case $control_status:$control_state in
0:controlled | 1:provenance-only) ;;
3:*) exit 3 ;;
*)
