...
PASS keys. PR #101's `__builtin_prefetch` tranche is merged as `79e41879`,
...
target-complete PASS keys across six compile and execute files. PR #104's
libc-compatible `__builtin_malloc`/`__builtin_free` pair is merged as
`caba647c`, publishing ninety target-complete PASS keys across nine files. PR
...
green before the green-only merge. PR #114's
`s56.19-builtin-fp-abs-family` tranche is merged as `c2a8023c`. It implements
`__builtin_fabs`, `__builtin_fabsf`, and `__builtin_fabsl` with exact
prototypes, target-softfloat constant evaluation, payload-preserving sign
clearing, and native x86-64/ARM64 lowering. It publishes twenty
target-complete PASS keys and leaves the ratchet at 31,060 PASS keys (31,063
lines). Its final standard, bootstrap, and exact-merge nightly CI were fully
green before the green-only merge, and the actual merge tree is byte-identical
to the tested synthetic merge. PR #115's
`s56.20-builtin-fp-classification-family` tranche is merged as `e3f90f14`.
It implements type-generic `__builtin_isnan`, `__builtin_isinf`,
`__builtin_isfinite`, and `__builtin_signbit`, publishes fifty
target-complete PASS keys, and leaves the ratchet at 31,110 PASS keys (31,113
lines). Final standard, bootstrap, and exact-merge nightly CI were fully green
before the green-only merge; the actual merge tree is byte-identical to the
tested final synthetic merge. PR #116's
`s56.21-builtin-fp-copysign-family` tranche is merged as `7f09b8a2`. It implements
`__builtin_copysign`, `__builtin_copysignf`, and `__builtin_copysignl` with
exact prototypes, target-softfloat constant evaluation, payload-preserving
sign copying, and native x86-64/ARM64 lowering. Focused semantic, IR, runtime,
sanitizer, deterministic-fuzz, oracle, and cross-target validation are green.
The exact synthetic merge `b729689b` passed the complete bootstrap lattice in
[run 34844539529](https://github.com/tenseleyFlow/Cgfried/actions/runs/34844539529).
Its pre-publication standard and exact-merge nightly completed with every
ordinary job green and only the intended x86 and ARM ratchet failures. Atomic
two-target publication adds exactly 60 keys, raising the ratchet to 31,170
PASS keys (31,173 lines) with 2,870 classified failures, 32 buckets, 24
...
without changing the ratchet. PR #120's
`s56.25-builtin-formatted-output` tranche is merged as `849b3015`. It
implements `__builtin_printf`, `__builtin_sprintf`, and
...
green before the green-only merge, and the actual merge tree is byte-identical
to the tested final synthetic merge. PR #122's
`s56.27-builtin-strncpy` tranche is merged as `5917a8d5`; it implements
libc-compatible `__builtin_strncpy`, repairs the GNU-inline self-binding case
...
- `.docs/audits/bootstrap-soak.md` is **RUNNING at a strict 16/30**. The first streak
  ...
  reproducibility, and cross-host comparison green. Scheduled September 14
  [run 34825113796](https://github.com/tenseleyFlow/Cgfried/actions/runs/34825113796)
  passed all four required daily jobs at exact head `2b193f3c` and is day 6;
  no weekly work was due. Scheduled September 15--19 runs `34948243637`,
  `35074563332`, `35200797830`, `35323369962`, and `35430928573` are days
  7--11, each with all four daily jobs and artifacts green. September 20 daily
  [run 35499954218](https://github.com/tenseleyFlow/Cgfried/actions/runs/35499954218)
  and full weekly
  [run 35500570594](https://github.com/tenseleyFlow/Cgfried/actions/runs/35500570594)
  share exact head `30afd68a` and make day 12; all four daily jobs,
  reproducibility, cross-host comparison, and eight retained weekly artifacts
  ...
  are unexpired and digest-stamped. September 24 scheduled
...
- The merged `s56.6-builtin-prefetch` tranche (PR #101) promotes
  ...
  Final post-publication standard
  [run 34524173872](https://github.com/tenseleyFlow/Cgfried/actions/runs/34524173872),
  bootstrap
  [runs 34524168989](https://github.com/tenseleyFlow/Cgfried/actions/runs/34524168989)
  and
  [34524173848](https://github.com/tenseleyFlow/Cgfried/actions/runs/34524173848),
  and exact synthetic-merge native-ARM
  [run 34524319980](https://github.com/tenseleyFlow/Cgfried/actions/runs/34524319980)
  are green. PR #101 merged as `79e41879`.
- The merged `s56.7-builtin-strcmp` tranche (PR #102) promotes
  `__builtin_strcmp` from the reserved-builtin refusal to a compiler-owned
  alias with the libc prototype `int (const char *, const char *)`. Sema
  applies both parameter conversions and the `int` result type. Lowering emits
  a real `strcmp` call, while a same-named definition in the translation unit
  remains an internal function reference so call identity survives textual IR
  round trips and agrees with linker-symbol binding.

  Focused normal and ASan+UBSan semantic and lowering coverage is green. The
  executable fixture pins exactly-once operand evaluation, equal/less/greater
  results, unsigned-character ordering, and the emitted libc call. All 100
  header-independent candidate assembly-generation cells pass locally across
  both Linux targets and O0/O1/O2/O3/Os; the hosted `920726-1.c` case also
  passes in all five native Apple-silicon executions. The full eleven-file
  native set is 55/55 green. The complete native unit run reaches the unchanged
  eight-failure host-assumption baseline at 879 tests and 4,328,650 assertions,
  with both affected tests passing. Pinned clang-format 22, static seams,
  normal and sanitized builtin suites, all builtin program fixtures, frontend,
  preprocessor, differential, and IR fuzz smoke, and the crash ledger are
  green. The added fixture intentionally repins the deterministic frontend
  mutation digest to `ab1da1c8c52691bb`.

  The retained x86 stream from
  [run 34529640772](https://github.com/tenseleyFlow/Cgfried/actions/runs/34529640772)
  and exact synthetic-merge native-ARM stream from
  [run 34529681616](https://github.com/tenseleyFlow/Cgfried/actions/runs/34529681616)
  each contain 20,325 matrix rows and name revision
  `b1e1c862f5b87b0f74809dddddb77b0cf4c177d4`. They share compiler-source
  SHA-256 `858a93ec3f8e0521df8f3073f632fd25c1bb2f9d7a72c7ebcca11e2cb0944d5f`,
  harness SHA-256
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  torture-manifest SHA-256
  `2757eaa59a81868c7565a9ff745ffced6e6a01c01efa0d000bdf27949e360da0`,
  ...
  `859ef7266c1ce061c7ed659abd9a2bd2782902d5f4c96085ce35249ae7cddd7e`.
  X86 and ARM stream SHA-256 values are respectively
  `92f5e7b8be2779b0a8a061616e71f3a0c3138daa7cfd563d600652bb270b6d88`
  and `9d154db1d7c3a688bfda82b7c05e4ed5b98927e86296898d8f660fb68d99bf41`.
  Each gate rejects only its 55 unpublished candidate PASS cells, covering
  `pr30564-2.c`, `pr33382.c`, `20010915-1.c`, `920726-1.c`, `921006-1.c`,
  `921007-1.c`, `930725-1.c`, `pr44852.c`, `pr47337.c`, `pr56205.c`, and
  `pr60062.c`, with no old-PASS regression. The remaining affected failures
  advance deterministically to already-known follow-on gaps.

  The atomic publisher consumes the explicit evidence pair in both target
  orders and regenerates PASS plus triage byte-identically. Publication adds
  exactly 110 PASS keys and reduces the pre-triaged `gcc-builtin` class from
  1,500 to 1,380 cells: 110 become PASS, while the ten target-complete
  `split-path-7.c` cells advance to the GNU no-named-member refusal. The
  resulting ratchet has 30,500 PASS keys (30,503 lines), 3,540 classified
  failures across 32 buckets, 24 applied decisions, two deliberately retained
  stale decisions, no live repair rows, and zero unbucketed or unresolved
  cells. PASS and triage SHA-256 values are respectively
  `64ee4a404daac1548ad0378191387dd2c73ba74a91b82f0f71ddc013ed1045ec`
  and `0232d3dbec9fdd615a4697baf357a3a6e91e7b4d334244cd2dd78fb485936721`.
  Final post-publication standard
  [run 34531566520](https://github.com/tenseleyFlow/Cgfried/actions/runs/34531566520),
  bootstrap
  [runs 34531561693](https://github.com/tenseleyFlow/Cgfried/actions/runs/34531561693)
  and
  [34531566534](https://github.com/tenseleyFlow/Cgfried/actions/runs/34531566534),
  and exact synthetic-merge native-ARM
...
- The merged `s56.8-builtin-strcpy` tranche (PR #103) promotes
  ...
  real `strcpy` call and uses the same whole-module symbol binding as
  `__builtin_strcmp`, so a same-named definition remains an internal reference
  and textual IR round trips preserve call identity.

  Focused normal and ASan+UBSan semantic and lowering coverage is green. The
  executable fixture pins the original destination return pointer,
  exactly-once operand evaluation, nonempty and empty copies, and an emitted
  libc call. All builtin-focused normal and sanitized tests and all fifteen
  builtin program fixtures pass. All sixty independently unlockable candidate
  assembly-generation cells pass locally across both Linux targets and
  O0/O1/O2/O3/Os; all thirty native Apple-silicon executions pass. The complete
  native unit run reaches the unchanged eight-failure host-assumption baseline
  at 881 tests and 4,328,667 assertions, with both added tests passing. Pinned
  clang-format 22, static seams, frontend, preprocessor, differential, and IR
  fuzz smoke, and the crash ledger are green. The added fixture intentionally
  repins the deterministic frontend mutation digest to `8e7eb41c3941bdfe`,
  reproduced by two independent 5,000-iteration runs.

  The retained x86 stream from
  [run 34536408950](https://github.com/tenseleyFlow/Cgfried/actions/runs/34536408950)
  and exact synthetic-merge native-ARM stream from
  [run 34536473598](https://github.com/tenseleyFlow/Cgfried/actions/runs/34536473598)
  each contain 20,325 matrix rows and name revision
  `fc057c88b4be4499a87a629ed51e17323e452ae7`. They share compiler-source
  SHA-256 `c736a490ac2c2e2f2b71f7e747f211bcdbba19eeef7e4078a3b9d075673555c4`,
  harness SHA-256
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  torture-manifest SHA-256
  `2757eaa59a81868c7565a9ff745ffced6e6a01c01efa0d000bdf27949e360da0`,
  ...
  `859ef7266c1ce061c7ed659abd9a2bd2782902d5f4c96085ce35249ae7cddd7e`.
  X86 and ARM stream SHA-256 values are respectively
  `3380103c1d0ebccda7107918d4c9907f9b7cd31dc950ed61aa024f00e1b9b072`
  and `90f4d23fb44226fc85a9eeb313d8a1436c837d45ffb9d2f04fbdcf0a9ad3e3cf`.
  Each gate rejects only its thirty unpublished candidate PASS cells, covering
  `strcpy-{1,2}.c` in the compile suite and `20021011-1.c`, `921117-1.c`,
  `pr86528.c`, and `strcpy-2.c` in the execute suite, with no old-PASS
  regression. The ten target-complete `strlen-7.c` failures advance from
  `__builtin_strcpy` to the follow-on `__builtin_malloc` gap.

  GNU Make 4.4.1 consumes the explicit evidence pair in both target orders and
  regenerates PASS plus triage byte-identically. Publication adds exactly sixty
  PASS keys and reduces the pre-triaged `gcc-builtin` class from 1,380 to 1,320
  cells. The resulting ratchet has 30,560 PASS keys (30,563 lines), 3,480
  classified failures across 32 buckets, 24 applied decisions, two deliberately
  retained stale decisions, no live repair rows, and zero unbucketed or
  unresolved cells. PASS and triage SHA-256 values are respectively
...
- The current `s56.11-builtin-abs-family` tranche (PR #106) completes
  ...
  Final post-publication standard
  [run 34558158852](https://github.com/tenseleyFlow/Cgfried/actions/runs/34558158852),
  bootstrap
  [runs 34558155998](https://github.com/tenseleyFlow/Cgfried/actions/runs/34558155998)
  and
  [34558158868](https://github.com/tenseleyFlow/Cgfried/actions/runs/34558158868),
  and exact synthetic-merge native-ARM
  [run 34558171987](https://github.com/tenseleyFlow/Cgfried/actions/runs/34558171987)
  are green. The final x86 and ARM streams name synthetic revision
  `8dd2d69a1d47c1a51e33db9f84bda009a778eca3`, pass the published ratchet,
  and have SHA-256 values
  `36bf13c347542d5bd4736836260408d41eec3580f0590f923356fee140f3a933`
  and `77a4aada437049e9aa5f7eabfc57ef03065a09c575eaac0c336a57f565b3e52f`.
  PR #106 merged as `9e4bcebe`; its merge tree is identical to the checked
  synthetic merge.
- The current `s56.12-builtin-ffs-family` tranche (PR #107) implements
  `__builtin_ffs`, `__builtin_ffsl`, and `__builtin_ffsll` with their exact
  signed `int`, `long`, and `long long` parameter conversions and common
  `int` result type. Explicit builtin calls fold as integer constant
  expressions, including zero, negative values, and target-width high bits.
  Lowering evaluates the source operand once and performs a branchless binary
  search using target-neutral integer IR; no libc symbol or new backend opcode
  is required. Ordinary POSIX `ffs` calls remain external symbols.

  Focused normal and ASan+UBSan semantic and lowering coverage is green at two
  tests and twenty-five assertions. The executable fixture pins zero,
  positive, negative, high-bit, prototype-conversion, and exactly-once
  behavior. All nineteen builtin fixtures pass in normal and sanitized
  Apple-silicon builds. Across O0/O1/O2/O3/Os, five native Apple compile cells
  and twenty native compile-and-execute cells pass. The dual-Linux-target
  source matrix is 50/50 across five independently unlockable files:
  `BUG25.c`, `ffs-1.c`, `ffs-2.c`, `pr61725.c`, and `pr97325.c`. The combined
  `builtin-bitops-1.c` fixture advances through the implemented ffs family to
  its independent clz/ctz/clrsb/popcount/parity gaps. The complete native unit
  run reaches the unchanged eight-failure Apple host-assumption baseline at
  890 tests and 4,328,767 assertions, with both new tests passing. Pinned
  clang-format 22, pristine imports, static seams and policy gates, the full
  ordinary and sanitizer CI suites, and 100,000-case sanitizer-backed frontend
  fuzzing are green. The new fixture intentionally repins the deterministic
  frontend mutation digest to `4d10c51c75ae9b18`, reproduced twice normally
  and once under ASan+UBSan at 5,000 iterations; both 2,000-case local fuzz
  executions have zero findings and the crash ledger is clean.

  Pre-publication x86
  [run 34561704961](https://github.com/tenseleyFlow/Cgfried/actions/runs/34561704961)
  and exact synthetic-merge native-ARM
  [run 34561728299](https://github.com/tenseleyFlow/Cgfried/actions/runs/34561728299)
  each contain 20,325 matrix rows and name revision
  `ed0ff24748a694cf9af5c4a57a9a74cb9caaeb70`. They share compiler-source
  SHA-256 `df4eb03391395f3f1d98ebdb16bf0f4383d2f958d4a06ef47685abad7f77dcf6`,
  harness SHA-256
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  torture-manifest SHA-256
  `2757eaa59a81868c7565a9ff745ffced6e6a01c01efa0d000bdf27949e360da0`,
  ...
  `859ef7266c1ce061c7ed659abd9a2bd2782902d5f4c96085ce35249ae7cddd7e`.
  X86 and ARM stream SHA-256 values are respectively
  `89fda19125161fbed3532c4ea89899b65e3de8c3c1701abb16ca8571648ec156`
  and `7a166984717b866149ec70634294e12aba0a25a23b8b0aeef2b47a8bcf6c555d`.
  Each gate rejects only its twenty-five unpublished candidate PASS cells,
  covering the five files above at all optimization levels, with no old-PASS
  regression.

  The atomic publisher consumes the explicit evidence pair in both target
  orders and regenerates PASS plus triage byte-identically. Publication adds
  exactly fifty PASS keys and reduces the pre-triaged `gcc-builtin` class from
  1,110 to 1,060 cells. The resulting ratchet has 30,810 PASS keys (30,813
  lines), 3,230 classified failures across 32 buckets, 24 applied decisions,
  two deliberately retained stale decisions, no live repair rows, and zero
  unbucketed or unresolved cells. PASS and triage SHA-256 values are
  respectively
...
- The current `s56.15-builtin-clrsb-family` tranche (PR #110) implements
  ...
  committed PASS/triage ratchet. Final standard
  [run 34583675383](https://github.com/tenseleyFlow/Cgfried/actions/runs/34583675383),
  bootstrap
  [runs 34583671931](https://github.com/tenseleyFlow/Cgfried/actions/runs/34583671931)
  and
  [34583675819](https://github.com/tenseleyFlow/Cgfried/actions/runs/34583675819),
  and exact synthetic-merge native-ARM
  [run 34583694904](https://github.com/tenseleyFlow/Cgfried/actions/runs/34583694904)
  are green. The final x86 and ARM streams contain 20,325 matrix rows, name
  synthetic revision `6e0b79f787b654c20a0b0a323aae3be3c5d3483b`, share common
  provenance, pass the unchanged ratchet, and have SHA-256 values
  `d682d26f32bfd82d98fd9cc6378ee2733041a1b103abbc1df650e116b65803a4`
  and `825cd8b22ee0b61e501e2cb84478116d7a94e41f53e4d19480d92a58b33c0819`.
  PR #110 merged as `db681380`; the actual merge has parents `1b015704` and
  `b9e0b951` and tree `88bf36eb5bace7e961323f0f89d375920f3aec4d`,
  byte-identical to the tested synthetic merge.
- The current `s56.16-builtin-parity-family` tranche implements
  `__builtin_parity`, `__builtin_parityl`, and `__builtin_parityll` with exact
  unsigned `int`, `long`, and `long long` parameter conversions and common
  `int` results. Explicit calls fold as integer constant expressions at the
  converted target width, including zero. Runtime lowering evaluates its
  operand exactly once, reuses the target-neutral population-count reduction,
  and masks the resulting `int` count to its low bit. It requires neither
  libc symbols nor new IR/backend opcodes.

  Focused normal and ASan+UBSan semantic/lowering coverage is green at two
  tests and twenty-three assertions. All twenty-three builtin fixtures pass
  in both configurations. Full ordinary and sanitizer unit runs reach the
  unchanged eight Apple host-assumption failures at 898 tests and 4,328,856
  assertions, with both new tests passing. The runtime fixture passes Cgfried
  and Apple Clang at O0/O1/O2/O3/Os, all five supported Cgfried targets
  compile it at all five optimization levels, and 4,096 deterministic inputs
  per family agree with an independent shift-and-XOR reference. The original,
  unmodified `builtin-bitops-1.c` now executes successfully under Cgfried at
...
- The current `s56.19-builtin-fp-abs-family` tranche implements
  ...
  Publication commit `72824685` passed fresh post-publication standard
  [run 34828233603](https://github.com/tenseleyFlow/Cgfried/actions/runs/34828233603),
  branch and PR bootstrap
  [runs 34828229775](https://github.com/tenseleyFlow/Cgfried/actions/runs/34828229775)
  and
  [34828233782](https://github.com/tenseleyFlow/Cgfried/actions/runs/34828233782),
  exact synthetic-merge nightly
  [run 34828264919](https://github.com/tenseleyFlow/Cgfried/actions/runs/34828264919),
  and its matching bootstrap
  [run 34828265332](https://github.com/tenseleyFlow/Cgfried/actions/runs/34828265332).
  The final rollup has 24 successful and nine policy-skipped checks. PR #114
  merged as `c2a8023c`; the actual merge has parents `2b193f3c` and
...
- The current `s56.20-builtin-fp-classification-family` tranche implements
  ...
  real floating operand, retains its exact format, and returns `int`.
  Constant evaluation reads target-softfloat class/sign state and never the
  host FPU. Runtime lowering evaluates the operand once: `isnan` uses an
  unordered self-compare; `isinf` and `isfinite` compare the absolute value
  with target-format infinity; `signbit` inspects f32/f64 carriers directly
  and calls compiler-owned f80/f128 carrier helpers where the scalar IR has no
  exact-width integer bitcast.

  Focused normal, ASan+UBSan, and clean Linux unit runs pass two tests and 61
  assertions. The runtime fixture passes Cgfried and Apple Clang at
  O0/O1/O2/O3/Os on Apple silicon, and passes the Linux x86 and ARM-QEMU
  runners. Five unmodified GCC torture candidates pass 25/25 x86 cells and
  25/25 ARM cells across O0/O1/O2/O3/Os. The binary128 differential produces
  1,432 result lines byte-identical to libgcc and audits all 24 ABI entry
  points plus three compiler helpers. The intended fixture addition repins
  the 5,000-iteration frontend mutation digest to `158ea03d2f1b33f3`,
  reproduced under Apple normal, Apple ASan+UBSan, and clean Linux builds;
  2,000-case normal and sanitizer fuzz smoke runs report zero findings. The
  authoritative Linux unit suite passes 915 tests and 4,329,152 assertions,
  and the complete local Linux regression gate set is green. The exported VM
  copy needed a local Git identity for provenance tests, `zip` for the SQLite
  ...
  Publication commit `56f131c6` passed final standard
  [run 34838944223](https://github.com/tenseleyFlow/Cgfried/actions/runs/34838944223)
  with twenty successful jobs and one policy skip. Branch, PR, and exact-merge
  bootstrap
  [runs 34838940469](https://github.com/tenseleyFlow/Cgfried/actions/runs/34838940469),
  [34838944292](https://github.com/tenseleyFlow/Cgfried/actions/runs/34838944292),
  and
  [34838991154](https://github.com/tenseleyFlow/Cgfried/actions/runs/34838991154)
  are green, and final exact-merge nightly
  [run 34839003177](https://github.com/tenseleyFlow/Cgfried/actions/runs/34839003177)
  passes all fifteen jobs. PR #115 merged green-only as `e3f90f14`; the
  actual merge has parents `c2a8023c` and `56f131c6`, and tree
  `17b6d734658a8b3503f86f8bb8c4ccc5830fe5c8`, byte-identical to the fully
  tested final synthetic merge.
- The current `s56.21-builtin-fp-copysign-family` tranche implements GCC's
  fixed-prototype `__builtin_copysign`, `__builtin_copysignf`, and
  `__builtin_copysignl` operations. Sema converts both operands to the exact
  result format. Constant evaluation replaces only the sign field in
  target-softfloat data. Runtime lowering evaluates each operand once, takes
  the payload-preserving absolute and negative forms of the first operand,
  extracts the second operand's sign through the existing f32/f64 carriers or
  f80/f128 helpers, and selects the exact result. This preserves signed zero,
  infinity, and NaN payloads across all three target long-double ABIs without
  consulting the build host's FPU.

  Focused normal and ASan+UBSan semantic/IR tests pass two tests and seventy
  assertions. The new runtime fixture passes Cgfried at every optimization
  level on Apple silicon and Apple Clang at O0/O1/O2/O3/Os. It compiles to
  assembly for arm64-linux, x86_64-linux-gnu, and arm64-macos at all five
  levels. Four header-independent unmodified GCC candidates compile and lower
  in all forty Linux target/level cells; three of their runtime cases pass
  natively on Apple. The two complete upstream copy-sign stress programs
  require Linux's supported headers, so their authoritative execution remains
  with hosted torture CI. The intended fixture addition repins the 5,000-case
  frontend mutation sequence to `909a9bcba3797e11`, reproduced twice normally
  and once under ASan+UBSan; 2,000-case normal and sanitizer runs report zero
  findings. The exact pinned clang-format 22 gate and the 64-row format matrix
  are green. The complete Apple unit run passes every new test and retains the
  unchanged eight documented host-assumption failures.

  Pre-publication standard
  [run 34844427418](https://github.com/tenseleyFlow/Cgfried/actions/runs/34844427418)
  at behavior head `23d72df7` completed with every ordinary job green and only
  the intended x86 ratchet failure. Exact synthetic-merge nightly
  [run 34844536769](https://github.com/tenseleyFlow/Cgfried/actions/runs/34844536769)
  at `b729689b` likewise has all fourteen ordinary jobs green and only the
  intended native-ARM ratchet failure. Branch and PR bootstraps
  [runs 34844423109](https://github.com/tenseleyFlow/Cgfried/actions/runs/34844423109)
  and
  [34844427388](https://github.com/tenseleyFlow/Cgfried/actions/runs/34844427388)
  are green, and exact-merge
  [run 34844539529](https://github.com/tenseleyFlow/Cgfried/actions/runs/34844539529)
  passes the full x86 O0/O2, native ARM O0/O2, and cross-host comparison
  lattice.

  The x86 and ARM evidence streams share source revision `b729689b`, compiler
  source, harness, torture-manifest, and c-testsuite-manifest provenance. Each
  contains 20,325 unique cells: 15,585 PASS, 3,305 SKIP, and 1,435
  COMPILE_FAIL, with zero ICEs, duplicates, or old-PASS regressions. Each
  target contributes exactly thirty new PASS keys at O0/O1/O2/O3/Os across
  `pr118501.c`, `copysign1.c`, `copysign2.c`, `mzero6.c`, `pr72824.c`, and
  `pr44683.c`. The x86 and ARM result-stream SHA-256 hashes are respectively
  `d7581d23160d740a37a4257daea9ca09b5d3580b1de190a4a0141fb7c59d2274`
  and
  `73b3cfef530c1d83e8b53e15ad9e2eafac1ff763e7d8a5c084a0cbbe8552058b`.

  Formal GNU-make atomic publication consumes both streams and adds exactly
  sixty target-complete keys. Reversing the target order regenerates both
  outputs byte-identically, and the complete runner, triage, provenance,
  matrix, rollback, and import meta-suite passes. The published ratchet holds
  31,170 PASS keys (31,173 lines); triage contains 2,870 classified failures
  in 32 buckets, 24 applied decisions, two deliberately retained stale
  decisions, and zero unbucketed or unresolved cells. The `gcc-builtin` class
  falls from 760 to 700 failed cells. PASS and triage SHA-256 values are
  respectively
  `836cc28a8432ee27f604d63540cc7dfa8785d23ed3d604109cabc2701ca9ee83`
  and
  `a068edad90569b7c183bdfb92ade89fe62a894722d325bd133960c4d20a7b4c2`.
  Publication head `01bfff5c` passed final standard
  [run 34850624242](https://github.com/tenseleyFlow/Cgfried/actions/runs/34850624242)
  with twenty successful jobs and one policy skip, including the 39m42s
  100,000-case fuzz lane. Branch and PR bootstrap
  [runs 34850618324](https://github.com/tenseleyFlow/Cgfried/actions/runs/34850618324)
  and
  [34850624350](https://github.com/tenseleyFlow/Cgfried/actions/runs/34850624350)
  are green. Final exact synthetic-merge nightly
  [run 34850711482](https://github.com/tenseleyFlow/Cgfried/actions/runs/34850711482)
  passes all fifteen jobs, and matching full-lattice bootstrap
  [run 34850714564](https://github.com/tenseleyFlow/Cgfried/actions/runs/34850714564)
  passes all seven applicable jobs. PR #116 merged green-only as `7f09b8a2`;
  the actual merge has parents `e3f90f14` and `01bfff5c`, and tree
  `32e27066f742b6771b567175562b7e50f22c18dd`, byte-identical to tested
  synthetic merge `530d994f`.
- The current `s56.22-builtin-checked-overflow-store` tranche implements
  GCC's type-generic `__builtin_add_overflow`, `__builtin_sub_overflow`, and
  `__builtin_mul_overflow` operations. Sema retains each operand's independent
  integer type, requires a writable pointer to a non-boolean, non-enumerated
  integer result type, and returns exact `_Bool`. Lowering evaluates all three
  arguments once, represents both mathematical operands as unsigned
  sign-and-magnitude values, and checks the destination bounds without host
  arithmetic, i128 IR, or target-specific overflow operations. The stored
  result is formed modularly in an unsigned i64 carrier and converted to the
  exact qualified pointee type, avoiding both signed UB and unsupported x86
  byte-width multiplication.

  Focused normal, Apple-Clang, and ASan+UBSan semantic/IR tests pass two tests
  and forty-five assertions. The permanent runtime fixture exhausts all
  signed/unsigned-char add/subtract/multiply pairs for inputs from -128 to
  255, exercises 64-bit extrema, mixed signs, zero products, aliases, volatile
  stores, exact result type, and single evaluation, and passes Cgfried and
  Apple Clang at O0/O1/O2/O3/Os on Apple silicon. The complete builtin fixture
  directory passes 29/29 natively. Eleven unmodified GCC execute candidates
  pass 55/55 native level cells and compile/lower in all 110 x86-64/ARM64
  target-level cells. The fixture itself compiles at every level for
  x86_64-linux-gnu, arm64-linux, and arm64-macos; Apple Clang's integrated
  x86 assembler accepts its O0 ELF output. The intended permanent fixture
  repins the 5,000-case frontend sequence digest to `0fcae2637e88d382`,
  reproduced twice normally and once under ASan+UBSan. Normal and sanitizer
  2,000-case fuzz runs, 5,000-case IR fuzz, both 2,000-case preprocessor fuzz
  modes, pinned clang-format 22, and the 64-row/128-fixture format matrix are
  green.

  The first hosted standard run exposed one real x86-only defect in the new
  fixture: O0 lowering formed the destination's modular product as i8 and GNU
  `as` correctly rejected the nonexistent `imulb`. The implementation now
  forms every stored result in the unsigned i64 carrier before narrowing, and
  the IR unit explicitly forbids `mul i8`. Stale workflows and their evidence
  ref were cancelled and deleted. Corrected behavior head `eb712388` passes
  canonical Linux `make test` with 919 unit tests / 4,329,267 assertions and
  810/810 program fixtures, full ASan+UBSan, Linux ARM QEMU/native execution,
  macOS ARM64, strict GCC/Clang builds, warning, toolchain, campaign, safe, and
  formatting jobs in pre-publication standard
  [run 34858592448](https://github.com/tenseleyFlow/Cgfried/actions/runs/34858592448).
  Its 100,000-case sanitizer-backed fuzz lane passes in 38m28s, and the x86
  torture job refuses only the intended unpublished PASS set. Branch and PR bootstrap
  [runs 34858586371](https://github.com/tenseleyFlow/Cgfried/actions/runs/34858586371)
  and
  [34858592519](https://github.com/tenseleyFlow/Cgfried/actions/runs/34858592519)
  are green.

  Exact synthetic merge `31c961341fd841739a276e2297b738aecab5f548`
  has parents `7f09b8a2` and `eb712388`, and tree
  `0eb7d28d0f39a81576a977c56dd391dc13a857d4`. Its full-lattice bootstrap
  [run 34858621359](https://github.com/tenseleyFlow/Cgfried/actions/runs/34858621359)
  is green. Exact-merge nightly
  [run 34858618275](https://github.com/tenseleyFlow/Cgfried/actions/runs/34858618275)
  passes all fourteen ordinary jobs and refuses only the matching ARM ratchet.
  The two complete result streams share source revision `31c96134`, compiler
  source SHA-256
  `464818241c903ef40d4e2588eb81974c351890bc6fd3ab52d7f3bdf3ce04f100`,
  harness SHA-256
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  torture-manifest SHA-256
  `2757eaa59a81868c7565a9ff745ffced6e6a01c01efa0d000bdf27949e360da0`,
  and c-testsuite-manifest SHA-256
  `859ef7266c1ce061c7ed659abd9a2bd2782902d5f4c96085ce35249ae7cddd7e`.
  Each target has 20,325 unique cells: 15,640 PASS, 3,305 SKIP, and 1,380
  COMPILE_FAIL, with zero ICEs, duplicates, or old-PASS regressions. Each adds
  exactly 55 PASS keys at all five levels across `pr108789.c`, `pr64006.c`,
  `pr71554.c`, `pr84169.c`, `pr85095.c`, `pr89434.c`, `pr90311.c`,
  `pr91450-{1,2}.c`, `pr91635.c`, and `pr93494.c`. X86 and ARM stream SHA-256
  values are respectively
  `034b40b47669c25436fd6d9fdf84bd6d8f8f13364cb7431279aa106ea7dd77a6`
  and
  `42c5ad3c49824392f03bc3fdc0c76dec1f2bdfb8d54a44875981a8252dc2fce6`.

  Formal GNU-make atomic publication consumes both streams and adds exactly
  110 target-complete keys. Reversing target order regenerates both outputs
  byte-identically, and the complete runner, triage, provenance, matrix,
  rollback, and import meta-suite passes. The published ratchet holds 31,280
  PASS keys (31,283 lines); triage contains 2,760 classified failures in 32
  buckets, 24 applied decisions, two deliberately retained stale decisions,
  and zero unbucketed or unresolved cells. PASS and triage SHA-256 values are
  respectively
...
- PR #120's merged `s56.25-builtin-formatted-output` tranche implements
  ...
  Publication head `934a2375` passed final standard
  [run 34924555228](https://github.com/tenseleyFlow/Cgfried/actions/runs/34924555228)
  and PR bootstrap
  [run 34924555263](https://github.com/tenseleyFlow/Cgfried/actions/runs/34924555263).
  Final exact synthetic merge `31a928fbc50c6d0e8aef7c934a90001ffbeb05ca`
  passed exact-merge nightly
  [run 34924720329](https://github.com/tenseleyFlow/Cgfried/actions/runs/34924720329)
  and full-lattice bootstrap
  [run 34924720238](https://github.com/tenseleyFlow/Cgfried/actions/runs/34924720238).
  PR #120 merged green-only as
  `849b30150ca5da719a0a96ee30099a14821333c7`; its parents are `52de6c67`
  and `934a2375`, and its tree `ee72c061` is byte-identical to the tested
  final synthetic merge.
- The current `s56.26-builtin-strchr` tranche on PR #121 promotes
  `__builtin_strchr` from the reserved-builtin refusal to a compiler-owned
  alias with the exact hosted prototype `char *(const char *, int)`. Sema
  applies ordinary assignment conversions to both fixed arguments; lowering
  calls libc `strchr` and deliberately preserves a same-name definition in
  the current translation unit. The result is a pointer value on every
  target. Unit coverage proves exact arity and type checking, accepted scalar
  conversions, aggregate rejection, exactly-once lowering, `long`-to-`int`
  truncation, external and internal-symbol paths, and IR round trips.

  The permanent runtime fixture covers exact pointer results, unsigned-byte
  matching (`0x1ff` matches `0xff`), absent matches, the terminating NUL, and
  exactly-once evaluation. It passes natively on ARM64 macOS at
  O0/O1/O2/O3/Os. All 33 builtin fixtures pass both normally and under fresh
  ASan+UBSan builds. The complete Apple unit suite has 928 tests and
  4,329,419 assertions, retaining exactly the same eight documented
  host-assumption failures with both new tests green. The five broad Apple
  audit-fixture failures are also reproduced identically by an exact detached
  `trunk` control, so none is tranche-caused.

  Frontend fuzz passes 2,000 normal and 2,000 sanitizer mutations with zero
  findings and reproduces the intentionally repinned 5,000-case digest
  `7a708f374826caec` twice normally and once under sanitizers. Preprocessor
  fuzz passes 2,000 normal plus 2,000 differential cases; IR fuzz passes 5,000
  cases. Repository bans, unit registration, fuzz-crash ledger, POSIX shell,
  audit-sample, closeout metadata, and diff checks are green. Both candidate
  imports, `torture-compile/pr34029-2.c` and
  `torture-execute/pr69691.c`, pass all five optimization levels for both
  x86-64 Linux and ARM64 Linux assembly (20/20); `pr69691.c` also compiles,
  links, and runs natively on Apple Silicon at all five levels.

  Provenance-matched hosted x86 and ARM streams at exact synthetic merge
  `f37f8fa1924fd7e8d98953b7e5ae22ae83519a69` confirm exactly those twenty
  new target-complete PASS keys with zero old-PASS regressions, duplicates, or
  ICEs. Both share compiler-source SHA-256
  `cfc60cb99fb9e7dd2c18337f960fc1f2c5887c0420a1f97cb3ecd18019ab7a28`
  and unchanged harness/torture/c-testsuite manifest hashes. Each target has
  20,325 unique cells: 15,795 PASS, 3,305 SKIP, and 1,225 COMPILE_FAIL.
  Hosted x86 and ARM stream SHA-256 values are respectively
  `f9a6b33270df386f2f36f39efc51a0c1888546188e866277ba78044e066cdd5b`
  and
  `c2ad5e7f543da728351a746bb867b757e67619369a55fdd33145ed4c95edc3a5`.
  Pre-publication PR CI
  [run 34928180097](https://github.com/tenseleyFlow/Cgfried/actions/runs/34928180097)
  passes every ordinary job, including the complete sanitizer fuzz lane, and
  refuses only the intended unpublished x86 ratchet improvement. PR bootstrap
  [run 34928180043](https://github.com/tenseleyFlow/Cgfried/actions/runs/34928180043)
  is green. Exact-merge nightly
  [run 34928229415](https://github.com/tenseleyFlow/Cgfried/actions/runs/34928229415)
  passes every ordinary campaign job and refuses only the intended ARM
  ratchet; exact full-lattice bootstrap
  [run 34928229487](https://github.com/tenseleyFlow/Cgfried/actions/runs/34928229487)
  passes all applicable jobs.

  The first local formal publication attempt had built only `build/cgfried`,
  not `build/arm64-linux/libcgf_rt.a`; its resulting 120 binary128-helper link
  failures were setup-invalid, not compiler evidence. After explicitly
  building `make CC=gcc RT_TARGET=arm64-linux rt`, the complete local ARM
  result body is byte-identical to hosted ARM. Formal GNU-make publication
  with hosted x86 succeeds, direct publication from both hosted streams
  succeeds, and reversed target order produces byte-identical artifacts. Both
  target gates and the complete runner, triage, provenance, matrix, rollback,
  and import meta-suite pass. Atomic publication adds exactly twenty
  target-complete keys, raising the ratchet to 31,590 PASS keys (31,593 lines)
  and leaving 2,450 failures fully classified in 32 buckets, with 24 applied
  decisions, two deliberately retained stale decisions, zero unbucketed
  cells, and zero unresolved buckets. PASS and hosted-provenance triage
  SHA-256 values are respectively
...
- The current `s56.27-builtin-strncpy` tranche promotes
  ...
  Publication commit `935ba742` passed final standard
  [run 34943944485](https://github.com/tenseleyFlow/Cgfried/actions/runs/34943944485)
  with all twenty jobs green. Branch and PR bootstrap
  [runs 34943940845](https://github.com/tenseleyFlow/Cgfried/actions/runs/34943940845)
  and
  [34943944468](https://github.com/tenseleyFlow/Cgfried/actions/runs/34943944468)
  are green. Final exact synthetic merge
  `0e3f3c577e990fd4a5657e37a09f2eca340ceb5d` has tree
  `3efe479ac3d9999a84a20f2256b331e7cf005d1d`, byte-identical to the
  publication head; exact nightly
  [run 35010591922](https://github.com/tenseleyFlow/Cgfried/actions/runs/35010591922)
  passed all fifteen jobs and full-lattice bootstrap
  [run 35010591773](https://github.com/tenseleyFlow/Cgfried/actions/runs/35010591773)
  passed all seven jobs. PR #122 merged green-only as
  `5917a8d528af909fecf6e9cf8463d75045160cbc`; the actual merge has the
  exact tested parents and tree.
- The current `s56.28-builtin-stpcpy` tranche promotes `__builtin_stpcpy`
  from the reserved-builtin refusal to a compiler-owned alias with the exact
  hosted prototype `char *(char *, const char *)`. Sema applies ordinary
  assignment conversions to both arguments and returns `char *`. Lowering
  evaluates each argument exactly once and emits a real `stpcpy` call. A
  same-TU definition owns the symbol normally, while an inline-only self-named
  wrapper retains the external fallback introduced by the preceding tranche.

  Focused semantic and lowering tests pass two tests and 27 assertions. They
  cover exact arity and result type, accepted `void *` conversion, aggregate
  rejection at both argument boundaries, exactly-once evaluation, external
  and same-TU internal calls, GNU-inline self-binding, and textual IR round
  trips. The permanent runtime fixture proves terminator-pointer return,
  copied bytes, empty-source behavior, and exactly-once arguments; it passes
  Cgfried and Apple Clang at O0/O1/O2/O3/Os on Apple Silicon. All 35 builtin
  fixtures pass normally, and the focused unit and runtime tests pass under
  ASan+UBSan.

  The three independent imported candidates -- `pr38343-2.c`, `pr38343.c`,
  and `pr43636.c` -- compile and produce assembler-accepted output for both
  x86-64 Linux and ARM64 Linux at all five optimization levels (30/30), and
  produce 15/15 native ARM64 macOS objects. `20030518-1.c` advances on both
  Linux targets from `__builtin_stpcpy` to its separate
  `__builtin_mempcpy` gap. Exhausting every explicit imported use finds only
  those four compile sources plus the deliberate DejaGNU multi-source or `.x`
  control skips; the two multi-source runtime programs pass manual O0
  execution on Apple ARM64.

  The complete Apple unit suite has 933 tests and 4,329,486 assertions,
  retaining exactly the same eight documented host-assumption failures with
  both new tests green. Frontend fuzz passes 2,000 normal and 2,000 sanitizer
  mutations with zero findings and reproduces the intentionally repinned
  5,000-case digest `ae613e8a7739eb35` twice normally and once under
  sanitizers. Preprocessor fuzz passes 2,000 normal plus 2,000 differential
  cases, and IR fuzz passes 5,000 cases both normally and under sanitizers.
  Repository bans, unit registration, fuzz-crash ledger, POSIX shell,
  campaign metadata, import provenance, target-sema, verifier-coverage,
  no-host-FPU, local clang-format 23, and diff checks are green. Hosted format
  with exact clang-format 22 is green.

  Implementation commit `d8355679ee766b2638e99314cdebfa9ca55e58ec` is on
  PR #123. Pre-publication standard
  [run 35014240983](https://github.com/tenseleyFlow/Cgfried/actions/runs/35014240983)
  passed all nineteen applicable ordinary jobs, including its 100,000-case
  sanitizer-backed frontend fuzz lane, skipped only the tag-only policy job,
  and refused only the expected fifteen unpublished x86 PASS cells. Branch and
  PR bootstrap
  [runs 35014174119](https://github.com/tenseleyFlow/Cgfried/actions/runs/35014174119)
  and
  [35014240953](https://github.com/tenseleyFlow/Cgfried/actions/runs/35014240953)
  are green.

  Exact synthetic merge `6d40004f6905f723d9e40dc9e7f02e95a1c33742`
  has parents `5917a8d5` and `d8355679`, and tree
  `45687fb650dba3bc8d2a753ed7000b29d4991ecc`, byte-identical to the
  implementation commit. Its exact-merge nightly
  [run 35015665394](https://github.com/tenseleyFlow/Cgfried/actions/runs/35015665394)
  passed all fourteen ordinary jobs and refused only the matching fifteen
  native ARM PASS cells. Matching full-lattice bootstrap
  [run 35015665487](https://github.com/tenseleyFlow/Cgfried/actions/runs/35015665487)
  passed all seven jobs.

  The publishable x86 and ARM streams name that exact merge and share compiler
  source, harness, torture-manifest, and c-testsuite-manifest SHA-256 values
  `7d7b255ed941bf7dc739eaca63eb5f67bf0037c657dbb4bcb84807a285f1b47b`,
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  `2757eaa59a81868c7565a9ff745ffced6e6a01c01efa0d000bdf27949e360da0`,
  and
  `859ef7266c1ce061c7ed659abd9a2bd2782902d5f4c96085ce35249ae7cddd7e`.
  Each target contains 20,325 unique cells: 15,825 PASS, 3,305 SKIP, and
  1,195 COMPILE_FAIL, with zero ICEs, duplicate keys, or old-PASS regressions.
  X86 and ARM stream SHA-256 values are respectively
  `05a0ef449a34d04d8ed59540498080bb922a20a927e128a7920435634746113b`
  and
  `da93ae3fae61531822e051b72a7bcefe65d968b139cf29e51a9d006ab589acac`.

  Formal GNU-make atomic publication consumes both streams and adds exactly
  thirty target-complete keys. Reversing target order regenerates PASS and
  triage byte-identically; both published-stream gates and the complete
  runner, triage, provenance, matrix, rollback, and import meta-suite pass.
  The ratchet now holds 31,650 PASS keys (31,653 lines); triage contains 2,390
  classified failures in 32 buckets, 24 applied decisions, two deliberately
  retained stale decisions, and zero unbucketed or unresolved cells. The
  `gcc-builtin` class falls from 240 to 210 failed cells. PASS and triage
  SHA-256 values are respectively
...
- The merged `s56.39-builtin-extract-return-addr` tranche implements GCC's
  ...
  corpus/optimization objects against the x86-64/SSE2 ceiling.

  Corrected pre-publication standard
  [run 35500793298](https://github.com/tenseleyFlow/Cgfried/actions/runs/35500793298)
  passes all 23 executed non-torture jobs, including 100,000-case frontend
  fuzz, and skips only the tag-only policy job; its sole refusal is the five
  unpublished x86-64 cells. GitHub's exact synthetic merge
  `243409b892f5860b2ac30331bf9f45eb3f0ad07a` has parents `30afd68a` and
  `368a0c22`, and tree `0e12396074dfd7cba8b9b9db7590073d9c32d926`,
  byte-identical to the tranche head. Its exact nightly
  [run 35501369524](https://github.com/tenseleyFlow/Cgfried/actions/runs/35501369524)
  passes all fourteen non-torture jobs and refuses only the matching five
  ARM64 cells. Its exact full-lattice bootstrap
  [run 35501369486](https://github.com/tenseleyFlow/Cgfried/actions/runs/35501369486)
  is green in all seven jobs.

  The downloaded exact-merge x86 and ARM streams have SHA-256 values
  `96ad84402c381cca72fff31cb1e82574b1035848914a86904b6d626e0cd7770f`
  and
  `f0e3421587e590e24f8ddbd4ea6ec626b72e8cd2121110c5d876301d7b31c6f8`.
  Each contains 20,325 unique cells -- 15,855 PASS, 3,305 SKIP, and 1,165
  COMPILE_FAIL -- and contributes exactly five new `20040202-1.c` PASS keys
  with zero old-PASS regressions, duplicate keys, or ICEs. Both share source
  revision `243409b8`, compiler-source SHA-256
  `74544b1b0d0bd9f2f523f278c25d7f99aca4171688f3902b98e2aaede550749e`,
  harness SHA-256
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  and identical manifest hashes.

  Formal GNU-make atomic publication consumes those exact streams while
  retaining the publisher's provenance, complete-key, staging, and rollback
  ...
  meta-suite is green. The result is 31,710 PASS keys (31,713 lines), 2,330
  classified failures in 31 buckets, 24 applied decisions, two deliberately
  retained stale decisions, and zero unbucketed or unresolved cells. The
  pre-triaged `gcc-builtin` class falls from 160 to 150 failed cells. PASS and
  triage SHA-256 values are respectively
  `b76fb71f6998044f5959683c843bf7630c5850cce54c0a0af2981592d49ce40b`
  and
  `5a74f9b6220d544e6a4f4b746457b4233e2a23227e4ae62ed477692bf6467c56`.
  Final post-publication standard
  [run 35502691007](https://github.com/tenseleyFlow/Cgfried/actions/runs/35502691007)
  is green in all 28 applicable PR checks; nine event/policy checks are
  intentionally skipped. PR bootstrap
  [run 35502690998](https://github.com/tenseleyFlow/Cgfried/actions/runs/35502690998)
  and branch bootstrap
  [run 35502688987](https://github.com/tenseleyFlow/Cgfried/actions/runs/35502688987)
  are green. GitHub's final exact synthetic merge `e6e3ea7805ec64479a6103cdc91b0543cb6d3dc8`
  has parents `30afd68a` and `86e8f7ed`, with tree
  `f8415653d0e3230ee6d7ac526ba6d491edf094e5`, byte-identical to the
  published PR head. Its final native-ARM nightly
  [run 35666629041](https://github.com/tenseleyFlow/Cgfried/actions/runs/35666629041)
  passed all 15 jobs, and its ARM result stream has SHA-256
  `cfcb3749856f86609b85afd35b4b86fe237ae81c471854f583fdac8d506bd051`;
  it names that exact merge revision, passes the published ratchet gate, and
  includes all five `20040202-1.c` ARM PASS cells. The final full-lattice
  bootstrap
  [run 35666628951](https://github.com/tenseleyFlow/Cgfried/actions/runs/35666628951)
...
- PR #135's merged `s56.40-builtin-mempcpy` tranche was developed in an isolated linked
  ...
  `mempcpy` symbol. Focused semantic/lowering tests pass normally and with
  ASan+UBSan (2 tests, 23 assertions). The permanent executable fixture
  matches Apple Clang and Cgfried at O0/O1/O2/O3/Os on native ARM64 macOS;
  imported `20030518-1.c` emits assembler-accepted output in all 15
  x86-64/ARM64 Linux/ARM64 macOS cells. Full Apple units retain exactly eight
  documented host-assumption failures among 941 tests, with both new tests
  green. Normal and sanitized frontend fuzz each pass 2,000 mutations with
  zero findings. Policy, unit registry, crash corpus, closeout, and pinned
  clang-format 22 gates are clean.

  Pre-publication standard
  [run 35668999504](https://github.com/tenseleyFlow/Cgfried/actions/runs/35668999504)
  passes all 23 non-torture jobs, including 100,000-case frontend fuzz, skips
  only the tag-only policy job, and refuses only five unpublished x86 PASS
  cells. GitHub's exact synthetic merge
  `2cf224cd845c1a29b2e0164b36a6c15c6384f4d7` has parents `81ed8d21`
  and `dc598625`, and tree `41ac34371363354ce6e4a84feda1014eb3be0fd9`,
  byte-identical to the feature head. Its native ARM nightly
  [run 35669088303](https://github.com/tenseleyFlow/Cgfried/actions/runs/35669088303)
  passed all fourteen non-torture jobs and refused only the corresponding
  five unpublished ARM cells. Exact full-lattice bootstrap
  [run 35669088590](https://github.com/tenseleyFlow/Cgfried/actions/runs/35669088590)
  passed all seven jobs.

  The retained x86 and ARM streams have SHA-256 values
  `c24648128c03849079c1bd8364552c9bccd7a35d2cf76edac7fa428e2d27c40e`
  and
  `c4b0d98eee0029ef2036ff376ae4d1af7e8f5943815b42cc3bf7a44ad3e18466`.
  Each contains 20,325 unique cells -- 15,860 PASS, 3,305 SKIP, and 1,160
  COMPILE_FAIL -- with exactly five new `20030518-1.c` PASS keys and no old
  PASS regression. Both name source revision `2cf224cd`, compiler-source
  SHA-256
  `fd9b58baf7c8964f155f68c6940d2d8450147a9b2a0cc79ef60e3cb5a66b069d`,
  harness SHA-256
  `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  and identical manifest hashes. Formal GNU-make atomic publication consumes
  the exact streams, passes provenance/completeness/rollback checks, and is
  byte-identical in reversed input order. Both published streams pass the
  ratchet gate and the complete torture meta-suite is green. The resulting
  31,720 PASS keys (31,723 lines) leave 2,320 failed cells in 31 classified
  buckets, 24 applied decisions, two retained stale decisions, and zero
  unbucketed/unresolved cells. `gcc-builtin` falls from 150 to 140. PASS and
  triage SHA-256 values are respectively
...
- The completed `s56.43-builtin-strcspn` tranche is merged through PR #138.
  ...
  2,000-case frontend fuzz each report zero findings. An independent x86 Linux
  VM matches the executable output and accepts `pr119226.c` at all five
  levels; the closed x86-64/SSE2 ISA gate passes exactly 708 objects (118
  fixtures times six levels).

  Pre-publication GitHub exact synthetic merge
  `142d6d29d65620cdb83725f966149be2fdb5b157` has parents `2451ae45`
  and `3d9cac96` and tree `ec291fcdf1358e9c3368005c984d6d8372704441`,
  byte-identical to the feature head. Standard
  [run 35686351498](https://github.com/tenseleyFlow/Cgfried/actions/runs/35686351498)
  ...
  [run 35686372057](https://github.com/tenseleyFlow/Cgfried/actions/runs/35686372057)
  passes all fourteen non-torture jobs and refuses only the five matching
  ARM PASS cells. Exact full-lattice bootstrap
  [run 35686371872](https://github.com/tenseleyFlow/Cgfried/actions/runs/35686371872)
  passed all seven.

  The retained x86 and ARM streams have SHA-256 values
  `75ea1c46ae343a7b50e329b6e4af9b70e3e0c37669802502d2383e4604e3446b`
  and `ea6a6f3ceaa0d7b2648eed5237428abd2a9065b1218839261dc031bdee61331c`.
  Each retains 20,325 unique cells -- 15,875 PASS, 3,305 SKIP, and 1,145
  COMPILE_FAIL -- with exactly five new `pr119226.c` PASS keys and no old
  PASS regression. Both name source revision `142d6d29`, compiler-source
  SHA-256 `d6c7bb4b50a3e50c78754026fcd818a34af50402c57f43bf9f7f39747e6bd2fa`,
  harness SHA-256 `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  and identical manifest hashes. Formal GNU-make atomic publication consumes
  the exact streams and passes provenance, complete-key, staging, and
  rollback checks. Reversing stream order regenerates both outputs
  byte-identically; both published streams pass the ratchet gate and the
  complete torture meta-suite is green. The result is 31,750 PASS keys
  (31,753 lines), 2,290 failed cells in 31 classified buckets, 24 applied
  decisions, two retained stale decisions, and zero unbucketed/unresolved
  cells. `gcc-builtin` falls from 120 to 110. PASS and triage SHA-256 values
  are respectively
...
- The current isolated `s56.45-builtin-strncmp` tranche is based on merged
  ...
  Pre-publication GitHub exact synthetic merge
  `9e4545558ba62d3ad82a5d096b3bf7a753d1032d` has parents `9d229021`
  and `91644a09` and tree `03a460184138631eda0a8399c0122e87154159a8`,
  byte-identical to the feature head. Standard
  [run 35796377884](https://github.com/tenseleyFlow/Cgfried/actions/runs/35796377884)
  passed all 23 non-torture jobs, including its 100,000-case fuzz lane; its
  only refusal is five unpublished x86 PASS cells. Native ARM nightly
  [run 35796440332](https://github.com/tenseleyFlow/Cgfried/actions/runs/35796440332)
  passed all fourteen non-torture jobs and refused only the five matching ARM
  PASS cells. Exact full-lattice bootstrap
  [run 35796440253](https://github.com/tenseleyFlow/Cgfried/actions/runs/35796440253)
  passed all seven.

  The retained x86 and ARM streams have SHA-256 values
  `2fcc3dbf1fa6b99360de882848975206ba5aafe6315dadc9e61509ca13f5e6c5`
  and `a92a96bfed534e8aebe2dceb2a990c5e9eaf41e684e0fc62f2d7ec47551697e1`.
  Each retains 20,325 unique cells -- 15,885 PASS, 3,305 SKIP, and 1,135
  COMPILE_FAIL -- with exactly five new `pr106101.c` PASS keys and no old PASS
  regression. Both name source revision `9e454555`, compiler-source SHA-256
  `d62e97b77e77e0bb640464a1b5cbf2b8b0fb712a5a8e5d6f1045ff1b02a27ea4`,
  harness SHA-256 `6109f4d0bfa5a8e04b29e61c2bdf0ccb2d6eb2ef208c4fb16b8d1a2ac3dc957b`,
  and identical manifest hashes. Formal GNU-make atomic publication consumes
  the exact streams and passes provenance, complete-key, staging, and
  rollback checks. Reversing stream order regenerates both outputs
  byte-identically; both published streams pass the ratchet gate and the
  complete torture meta-suite is green. The result is 31,770 PASS keys
  (31,773 lines), 2,270 failed cells, 24 applied decisions, two retained stale
  decisions, and zero unbucketed/unresolved cells. `gcc-builtin` falls from
...
- The current isolated `s56.46-builtin-string-large-family` tranche is based
  ...
  Final standard
  [run 35922442797](https://github.com/tenseleyFlow/Cgfried/actions/runs/35922442797)
  passed all 24 executed jobs, with 28 PR check successes and nine intended
  skips. GitHub's exact published merge
  `8d4e68792a755ff458e6040aa470ca7379ab8aed` has parents `dffc958d`
  and `c2cf3f56` and tree `8c1675d795bba84d0f5e4b1662a5bd9ac7ffe920`,
  byte-identical to the published head. Native ARM nightly
  [run 35923077043](https://github.com/tenseleyFlow/Cgfried/actions/runs/35923077043)
  passed all fifteen jobs, and full-lattice bootstrap
  [run 35923077094](https://github.com/tenseleyFlow/Cgfried/actions/runs/35923077094)
  passed all seven. Final x86 and ARM streams name that exact merge, pass the
  committed ratchet, retain all 20,325 unique cells each, and have SHA-256
  values `f7b40f45d9b36dd1ffaaf77f4ca0b48281f772e4350230627d93fe0a1d975aef`
  and `cdea486ca82b8f2b11d7ad716e88771d920699648114a4dd3c645aa7b811db59`.
  PR #141 merged green-only as `32aabb17060422cbf0bed6630d4462a17410e6df`;
  its parents and tree are identical to the final tested merge.
- The current isolated `s56.47-builtin-pow` tranche is based on merged #141.
  ...
  exactly 732 corpus objects.

  Prepublication standard
  [run 35928632299](https://github.com/tenseleyFlow/Cgfried/actions/runs/35928632299)
  passed all 23 non-torture jobs, including its 100,000-case fuzz lane, and
  refused exactly the five unpublished x86 `pr110444-1.c` PASS cells.
  GitHub's exact synthetic merge
  `04d662801198b29b7bfd9f7783f10f50462ada89` has parents `32aabb17`
  and `01792ec2` and tree `6091a690254cbc464c4978efc8aadf60dec34bc4`,
  byte-identical to the feature head. Its native ARM nightly
  [run 35943870305](https://github.com/tenseleyFlow/Cgfried/actions/runs/35943870305)
  passed fourteen jobs and refused only the five matching ARM cells. Its
  manually dispatched full-lattice bootstrap
  [run 35944197842](https://github.com/tenseleyFlow/Cgfried/actions/runs/35944197842)
  passed all seven jobs.
  ...
  `c0126aac08f4dd266f141eabacc90764057c95d947af6085912f29f91b20bb6a`
  and `911db2b80fb5f482777f6218c4c8d0c562cf50c32d24e3cef39def53da6a3d3c`.
  Each contains 20,325 unique cells -- 15,895 PASS, 3,305 SKIP, and 1,125
  COMPILE_FAIL -- with exactly five new `pr110444-1.c` PASS keys and no old-
  ...
  metadata suite is green. The result is 31,790 PASS keys (31,793 lines),
  2,250 failed cells, 24 applied decisions, two retained stale decisions, and
  zero unbucketed/unresolved cells. `gcc-builtin` falls from 80 to 70. PASS
  and triage SHA-256 values are respectively
...
- The current isolated `s56.48-builtin-clear-cache` tranche is based on merged
  ...
  Final standard
  [run 35957512959](https://github.com/tenseleyFlow/Cgfried/actions/runs/35957512959)
  passes all 24 executed jobs. GitHub's final exact synthetic merge
  `a5feb357849b3e2e0518165203d3096d4d909266` has parents `d958f633`
  and `c6a566fb` and tree `78205108ce9bcaaae5aae9ea4626da6363f810e2`,
  byte-identical to the published head. Final native ARM nightly
  [run 35957924645](https://github.com/tenseleyFlow/Cgfried/actions/runs/35957924645)
  passes all fifteen jobs, and full-lattice bootstrap
  [run 35957931416](https://github.com/tenseleyFlow/Cgfried/actions/runs/35957931416)
  passes all seven. Final x86 and ARM streams name that exact merge, pass the
  committed ratchet, retain all 20,325 unique cells each, and have SHA-256
  values `a0c98a94537d9c062736cde3bae46018d59bf4b22bfd88c07f0a7b90e2e78b8c`
...
- The current isolated `s56.49-builtin-memset-chk` tranche is based on merged
  ...
  closeout, and POSIX-shell gates are green.

  Prepublication standard
  [run 36031591409](https://github.com/tenseleyFlow/Cgfried/actions/runs/36031591409)
  passed all 23 non-torture jobs, including its 100,000-case sanitizer-backed
  frontend fuzz lane, and refused only the expected five unpublished x86
  `20070915-1.c` cells. Branch and PR bootstrap
  [runs 36031558826](https://github.com/tenseleyFlow/Cgfried/actions/runs/36031558826)
  and
  [36031591383](https://github.com/tenseleyFlow/Cgfried/actions/runs/36031591383)
  are green. GitHub's exact prepublication synthetic merge is
  `d7fc634c80a2d942109c5a43204400661acfe819`, with parents `49f30d41`
  and `5dabc33f` and tree `7582e80521d43ac6904fbc8ea9e6a8ba64917fa6`,
  byte-identical to the feature head. Exact-merge native ARM nightly
  [run 36033858144](https://github.com/tenseleyFlow/Cgfried/actions/runs/36033858144)
  passed all fourteen unrelated jobs and refused only the matching five ARM
  cells. Exact-merge full-lattice bootstrap
  [run 36033861455](https://github.com/tenseleyFlow/Cgfried/actions/runs/36033861455)
  passed all seven jobs.
  ...
  `5ff259edab3a8a739e251d5afad1a1fbd8927ee3491ecb3a6a7ab21b8cbdd0f2`
  and
  `5c9c56a47b6e6eed311349f372da9bd94c7e30f2ceb07caaccea0daec199eceb`.
  Each contains 20,325 unique cells -- 15,905 PASS, 3,305 SKIP, and 1,115
  COMPILE_FAIL -- with exactly five new `20070915-1.c` PASS keys and no old-
  PASS regression. Formal GNU-make atomic publication consumes both exact
  streams; reversing input order regenerates both outputs byte-identically,
  both streams pass the published ratchet gate, and the complete torture
  metadata suite is green. The result is 31,810 PASS keys (31,813 lines),
  2,230 failed cells in 31 buckets, 24 applied decisions, two retained stale
  decisions, and zero unbucketed/unresolved cells. `gcc-builtin` falls from
  60 to 50. PASS and triage SHA-256 values are respectively
  `656c4e5a6f620717ebb567a4ad5755975da856afa183ef762cf6af0bc805134e`
  and `62af63f335820195eb057a15d8954649815c923d6b168cea0422b7ebe55a4ac8`.
  Publication commit `ad41ccf2` carries those deterministic outputs.

  Final standard
  [run 36038374558](https://github.com/tenseleyFlow/Cgfried/actions/runs/36038374558)
  passed all 24 executed jobs with one intended tag-only skip. GitHub's exact
  final synthetic merge `9ee6a0a577428aa3e4031cbd21d19cde5684ff05` has
  parents `49f30d41` and `ad41ccf2` and tree
  `1d940bd3ee7e2da5f7be0dc3cfa9eb5ff7fe9849`, byte-identical to the
  published head. Final native ARM nightly
  [run 36038862923](https://github.com/tenseleyFlow/Cgfried/actions/runs/36038862923)
  passed all fifteen jobs, and full-lattice bootstrap
  [run 36038865842](https://github.com/tenseleyFlow/Cgfried/actions/runs/36038865842)
  passed all seven. Final x86 and ARM streams name that exact merge, pass the
  committed ratchet, retain all 20,325 unique cells each, and have SHA-256
  values `2337558062bcce40bb42ab595ffd521f6de8a2263f4fe6e95a0392d850e05c4a`
