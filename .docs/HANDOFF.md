...
advances `20030518-1.c` to its separate `__builtin_mempcpy` gap. Commit,
hosted CI, exact two-target evidence, and ratchet publication remain pending.
...
- The current `s56.28-builtin-stpcpy` tranche promotes `__builtin_stpcpy`
  ...
  no-host-FPU, local clang-format 23, and diff checks are green. Exact hosted
  clang-format 22, the implementation commit/PR, complete hosted CI, exact
  target result streams, and ratchet publication remain pending.
